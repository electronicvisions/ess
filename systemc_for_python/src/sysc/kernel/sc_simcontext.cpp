...
#endif
	int outer = 0;		//debug consti
	int max_count = 10000;	//debug consti
    while ( true )
	{
		// EVALUATE PHASE
		m_execution_phase = phase_evaluate;
		int inner = 0; //debug consti
		while( true )
		{
			...
			//inner++;																										//debug consti
			//if(inner > max_count)																							//debug consti
			//{
				//std::cout << "sc_simcontext::crunch -> inner loop terminated at: " << inner << " counts" << std::endl;		//debug consti
				//inner = 0;
				//break;																										//debug consti
			//}
		...
		outer++;																									//debug consti
		if(outer > max_count)																						//debug consti
		{
			std::cout << "sc_simcontext::crunch outer loop terminated at:  " << outer << " count"  << std::endl;	//debug consti
			outer = 0;																								//debug consti
		    break;																									//debug consti
		}
