...
#define HC_FAST_SYS_CLK_PER_NS 5 ///< the period of the fast hicann system clock. used in merger_tree, priority encoder and background event generator. Corresponds to a PLL of 200Mhz.
...
#define L2_LABEL_WIDTH 12 ///< label size on point to point connection (HICANN<->DNC<->FPGA)
...
#define DELAY_MEM_DEPTH 512 ///<heap memory size, sorted memory for delaying at DNC ( for one HICANN channel )
...
#define DNC_TIME_LIMIT 2 ///< default time limit for transmission to hicann. Pulses to HICANN are released if difference between system counter and pulse event release time is equal to this value. Note, that all timestamps are shifted by 5 to the right. I.e. we compare multiples of 32 clock cycles.

#define LVDS_MODE_MULT 1 ///< multiplier for speed of the lvds links:  1 for fast (Gbit) and 2 flow slow (500Mbit)
#define TIME_SER_PULSE 24*LVDS_MODE_MULT ///< Serialisation time for pulse events in ns
#define TIME_DES_PULSE 24*LVDS_MODE_MULT ///< Deserialisation time for pulse events in ns
#define TIME_SER_CONFIG 44*LVDS_MODE_MULT ///< Serialisation time for config packets in ns
#define TIME_DES_CONFIG 44*LVDS_MODE_MULT ///< Deserialisation time for config packets in ns

#define CMP_TIMESTAMP_WIDTH_L2TOL1_TX 10  ///< number of LSBs of the timestamp of a pulse event that are compared to the current systime in the L2TOL1_TX

// System Simulation: Clock Periods
#define CLK_PER_DNC_IF 2 ///< Clock period of systemc module dnc_if [ns]
#define CLK_PER_L2TOL1_TX 4 ///< Clock period of systemc module l2tol1_tx [ns]
#define CLK_PER_L2_DNC 1 ///< Clock period of systemc module l2_dnc [ns]
#define CLK_PER_DNC_SER_CHANNEL 2 ///< Clock period of systemc module dnc_ser_channel [ns]
#define CLK_PER_FPGA_CHANNEL 2 ///< Clock period of systemc module fpga_channel [ns]
#define CLK_PER_L2_FPGA 1 ///< Clock period of systemc module l2_fpga [ns]
#define SYS_CLK_PER_L2_FPGA_NS 8 ///< Clock period of real FPGA [ns]
#define CLK_PER_DNC_TX_FPGA 2 ///< Clock period of systemc module dnc_tx_fpga [ns]
...
#define TIME_HYP_OUT_PULSE 2*LVDS_MODE_MULT	 ///< Time from FPGA to DNC, pulse events
#define TIME_HYP_IN_PULSE 2*LVDS_MODE_MULT	 ///< Time from DNC to FPGA, pulse events

#define TIME_HYP_OUT_CONFIG 8*LVDS_MODE_MULT ///< Time from FPGA to DNC, config packets
#define TIME_HYP_IN_CONFIG 8*LVDS_MODE_MULT  ///< Time from DNC to FPGA, config packets


#define L2_TRANSMIT_DELAY 500 ///< stimulus transmission delay in ns
#define L1_DELAY_REP_TO_DENMEM 80 ///< pulse delay between a repeater and a denmem in ns (L1 delay has been measured: neuron -> syndriver: 60 ns, syndriver ->neuron 60ns). The merger tree is modelled in the ESS, the rest not, thats why we stay with 80 ns. Note that this delay (Repeater to Denmem) is modelled at the neuron, i.e. spikes are released with this delay. This is the most efficient way to do it. eEvent the recorded spikes are not delayed, as the time of the input spikes was not considered before, but is now.
#define SYSTIME_PERIOD_NS 4.0 ///< time base of system time

//Simulation stuff for Wafer
#define HICANNS_PER_RETICLE 8			///< number of hicanns per reticle
#define RETICLE_X 9						///< assumption of 2d field of reticles per wafer X size
#define RETICLE_Y 8						///< assumption of 2d field of reticles per wafer Y size
...
#define HICANN_X 4						///< assumption of 2d field of hicanns per reticle X size
#define HICANN_Y 2						///< assumption of 2d field of hicanns per wafer Y size
...
#define FPGA_COUNT 12						///<number of fpgas in a wafer/PCB system
#define DNC_FPGA 4							///<how many dncs are connected to an fpga
#define DNC_COUNT (FPGA_COUNT * DNC_FPGA)	///<number of DNCs for ONE wafer/PCB system
...
#define SYNDRIVERS 112 ///< realistic value: 112 for hicann V2 or 124 for old hicann model, half (1/ROWS_PER_SYNDRIVER) of synapse row count (SYN_PER_ROW)
